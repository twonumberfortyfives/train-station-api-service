...
class Route(models.Model):
    ...
    def validate_source_destination(source, destination, error_to_raise):
        if source == destination:
            raise error_to_raise(f"Source and destination can not be the same")
...
class Ticket(models.Model):
    ...
    def validate_seat(
        ...
        if not (1 <= cargo <= train_cargo_num):
            raise error_to_raise(f"Cargo must be in range [1, {train_cargo_num}]")
        elif not (1 <= seat <= train_places_in_cargo):
            raise error_to_raise(f"Seat must be in range [1, {train_places_in_cargo}]")
